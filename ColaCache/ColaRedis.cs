...
public class ColaRedis : IColaRedisCache, IDisposable
{
    ...
    {
        if (options.CacheType != CacheType.Redis.ToInt() && options.CacheType != CacheType.Hybrid.ToInt()) return;

        #region 单机服务器

        if (options.RedisCache!.Standalone != null && !options.RedisCache.Standalone.ConnectionStrings.StringIsNullOrEmpty())
        {
        ...
        if (options.RedisCache.Sentinels != null && !options.RedisCache.Sentinels.ServiceName.StringIsNullOrEmpty() &&
            !options.RedisCache.Sentinels.Sentinel.StringIsNullOrEmpty())
        {
        ...
        if (options.RedisCache.Cluster != null && !options.RedisCache.Cluster.Masters.StringIsNullOrEmpty() &&
            !options.RedisCache.Cluster.Slaves.StringIsNullOrEmpty())
        {
        ...
        if (!options.RedisCache.EventBus.StringIsNullOrEmpty())
        {
